...
def _frame_capacities(total_horses: int) -> Sequence[Tuple[int, int]]:
    ...
    if total_horses <= 8:
        return [(frame, 1) for frame in range(total_horses, 0, -1)]
    if 9 <= total_horses <= 15:
        capacities: List[Tuple[int, int]] = []
        remaining = total_horses
        for frame in range(8, 0, -1):
            if remaining <= 0:
                break
        ...
        return capacities
    if total_horses == 16:
        return [(frame, 2) for frame in range(8, 0, -1)]
    if total_horses == 17:
        return [(8, 3)] + [(frame, 2) for frame in range(7, 0, -1)]
    if total_horses == 18:
        return [(8, 3), (7, 3)] + [(frame, 2) for frame in range(6, 0, -1)]
...
def _assign_post_positions(entries: List[HorseEntry]) -> None:
    ...
    descending_numbers = list(reversed(numbers))
    capacities = _frame_capacities(total)

    index = 0
    for frame, capacity in capacities:
        for _ in range(capacity):
            if index >= len(descending_numbers):
                break
            number = descending_numbers[index]
            entries_by_number[number].post_position = frame
            index += 1

    if index != len(descending_numbers):  # pragma: no cover - defensive programming
